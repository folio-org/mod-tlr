...
* Secondary request (TLR Hold) not canceled after primary request was cancelled (MODTLR-164)
* Remove `patronGroup` from request schema (MODTLR-150)
* Support items without barcodes (MODTLR-191)
* Add discoveryDisplayName in AllowedServicePoints response (MODTLR-202)
* Upgrade confluentinc/cp-kafka:7.5.3 to apache/kafka-native:3.8.0
...
* Update for Split Search & Browse APIs (MODTLR-222)
* Update item-storage interface version (MODTLR-221)
* Add dependabot (MODTLR-213)
* Declare item lost API (MODTLR-216, MODTLR-217)
...
* Use DCB location for circulation items (MODTLR-248)
* Fix JSON mapping and Jackson config (MODTLR-247)
* Exclude tenants from lending tenant search algorithm (MODTLR-251)
* Prevent patron from creating more than one ECS TLR (MODTLR-255)
* Get consortium configuration from mod-users (MODTLR-127)
* Do not cache null values in ConsortiumService (MODTLR-262)
* Handle EXPIRED status for DCB transactions (MODTLR-268)
* Remove potential PII leaks from logs (MODTLR-143)
* Migrate config values from mod-configuration (MODTLR-272)
* Repeat fetching when patron already exists (MODTLR-265)
* Enable GitHub Workflows (MODTLR-282)
* Upgrade to Spring Boot 4 and folio-spring-support 10 (MODTLR-263)
* Copy expiration date from secondary to primary/intermediate request (MODTLR-289)
